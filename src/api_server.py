...
def create_app(config_path: Path = DEFAULT_CONFIG) -> FastAPI:
    ...
    low_conf_region_candidate_rescue_max_rows = int(search_cfg.get("low_conf_region_candidate_rescue_max_rows", 6))
    ...
    low_conf_region_candidate_rescue_insert_after = int(search_cfg.get("low_conf_region_candidate_rescue_insert_after", 2))
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .meter-param-grid { width: 100%; display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); text-align: center; gap: 8px; font-size: 18px; line-height: 1.55; transform: translateY(-10px); }
    .meter-param-grid > div:last-child { padding-right: 58px; }
    .meter-param-title { font-size: 18px; margin-bottom: 6px; }
    ...
    .color-action-title { padding: 16px 18px 14px; background: #f8fafc; color: #0f172a; text-align: center; font-size: 16px; line-height: 1.45; font-weight: 700; word-break: break-word; border-bottom: 1px solid #edf0f4; }
...
<body>
  ...
  <script>
    ...
    function updateMeterPreview() {
      ...
      if (!box) return;
      box.closest(".meter-preview-card")?.classList.toggle("params-mode", state.meterPreviewMode === "params");
    ...
    async def search(
        ...
        with tempfile.NamedTemporaryFile(suffix=suffix, delete=True) as tf, tempfile.NamedTemporaryFile(suffix=".tight.jpg", delete=True) as tight_tf:
            ...
            def _cap_label_memory_for_checker_profile(profile: Dict[str, float] | None) -> bool:
                ...
                strong_checker_query = bool(
                    checker_value >= min_checker_for_cap
                    and bw_mix_value >= float(label_memory_checker_min_bw_mix)
                )
                if not base_code_prior_boost:
                    return False
                if strong_checker_query and not label_memory_checker_cap_enabled:
                    return False
                if not strong_checker_query and not label_memory_weak_checker_cap_enabled:
                    return False
                cap = max(
                    0.0,
                    float(label_memory_checker_max_boost)
                    if strong_checker_query
                    else float(label_memory_weak_checker_max_boost),
                )
